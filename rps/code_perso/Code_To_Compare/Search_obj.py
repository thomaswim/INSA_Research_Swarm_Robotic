...
close_enough = 0.05 ; #a quelle distance minimum du waypoint doit etre le follower afin de valider son étape
...
initial_conditions = np.array([[0, 0.5, 0.3, -0.1 ,0.1 ,0.2],[0.5, 0.5, 0.2, 0 ,0.6 ,0.4],[0, 0, 0, 0, 0, 0]])
...
marker_size_goal = determine_marker_size(r,0.05)
font_size_m = 0.1
font_size = determine_font_size(r,font_size_m)
line_width = 5

# Create goal text and markerscompleteGL
...
if stop ==0 :
	for t in range(iterations):
		...
		for i in range(0,N):
			...
			# SI wp trouvé ou que robot bloqué
			if ((waypoint_check[i] == 0) and Obj_find ==0) or Bloquage[i] > 1:
				xp = (random.random()*2)-1   ##A CHANGER POUR ADAPTER A PLATEAU
		...
		##Determiner si robot est bloqué ou si objectior i in dxu[0] :
		if Obj_find == 1 :
			for i in range(N) :
				if dxu[0][i] < 0.01 and dxu[1][i] < 0.01 :
					Fin[i] += 0.1
					
		else :
			for i in range(N) :
				if dxu[0][i] < 0.0001 and dxu[1][i] < 0.0001 :
					print("KEBLO ",i)
					Bloquage[i] += 0.1
					print(Bloquage[i])
					print (Bloquage[i])
